...
public class Move : MonoBehaviour
{
    ...
    private void Grounded()
    {
        ...
        {
            int layerMask = 1 << 6;
            RaycastHit2D hit = Physics2D.Raycast((Vector2)transform.position + Vector2.down * .9f + (Vector2.right * .5f), Vector2.down, 0.2f, layerMask);
            int layerMask2 = 1 << 9;
            RaycastHit2D hit2 = Physics2D.Raycast((Vector2)transform.position + Vector2.down * .9f + (Vector2.right * .5f), Vector2.down, 0.2f, layerMask2);
